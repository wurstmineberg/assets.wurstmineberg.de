    ...
    this.htmlImage = function(classes, tint) {
        if ('image' in itemInfo) {
            if (itemInfo.image.startsWith('http://') || itemInfo.image.startsWith('https://')) {
                ...
                return '<img style="background: url(' + (isDev ? 'http://devapi.wurstmineberg.de' : 'http://api.wurstmineberg.de') + '/minecraft/items/render/dyed-by-id/' + stringID.replace(/:/g, '.') + '/' + zeroFill(tint.toString(16), 6) + '/png.png' + itemInfo.image + ')" src="' + (isDev ? 'http://devassets.wurstmineberg.de' : 'http://assets.wurstmineberg.de') + '/img/grid-overlay/' + itemInfo.image + '" class="' + (classes || '') + '" />';
            }
        } else {
            return '';
        }
